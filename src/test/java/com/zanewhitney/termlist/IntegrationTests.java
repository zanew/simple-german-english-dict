...
public class IntegrationTests {
    ...
    public void getTerms_from1to3_ReturnsTerms() {
        ResponseEntity<Term> terms = this.testRestTemplate.getForEntity("/terms/{text}?n={resultsPerPage}", Terms.class, "geschehen", 3);

        assertThat(terms.getStatusCode()).isEqualTo(HttpStatus.OK);
        assertThat(terms.getBody().getTerms().size() == 3);

        Term occurred = terms.getBody().getTerms().get(0);
